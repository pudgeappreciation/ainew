...
pub type Loras = Arc<RwLock<Vec<Lora>>>;
...
pub async fn get() -> Vec<Lora> {
    ...
    for model_data in response.iter() {
        let Some(filename) = model_data.get("path") else {
            continue;
        };
        ...
        let Some(model) = Lora::from_path(Path::new(filename).to_path_buf()).await else {
            continue;
        };
